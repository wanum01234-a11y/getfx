...
import { mockOpenTrades, type Trade } from "@/lib/mock-data";
import { useDemoMode, useTrades } from "@/lib/mt5";
...
export default function PendingOrders() {
  ...
  const mt5 = useTrades("pending", !useDemo);
  const [shareOpen, setShareOpen] = useState(false);
  const [shareInitialId, setShareInitialId] = useState<string | undefined>(undefined);

  const trades = useMemo<Trade[]>(() => {
    if (useDemo) {
      return mockOpenTrades.map((t) => ({ ...t, status: "Pending" as const }));
    }
    return Array.isArray(mt5.data) ? (mt5.data as Trade[]) : [];
  }, [mt5.data, useDemo]);
    ...
    <Layout>
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white tracking-tight">Pending Orders</h1>
            <p className="text-muted-foreground mt-1">Orders placed but not triggered yet.</p>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl border border-white/10 overflow-hidden">
              ...
              <tbody>
                {trades.map((trade, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-muted-foreground">{trade.orderType ?? "-"}</td>
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 rounded-md text-xs font-semibold bg-yellow-500/10 text-yellow-300 border border-yellow-500/20">
                        Pending
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button
                        ...
                        aria-label={`Share ${trade.symbol} pending order via WhatsApp`}
                      >
          ...
          {trades.length === 0 && (
            <div className="p-8 text-center text-muted-foreground">No pending orders currently.</div>
          )}
        </div>
